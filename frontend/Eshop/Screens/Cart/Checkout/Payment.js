...
const Payment = props => {
    ...
    const order = props.route.params; // get data send by the route from Checkout.js page
            ...
            <Content>
                ...
                {selected === 3 ? (
                    <Picker
                        ...
                        selectedValue={card}
                        onValueChange={(selected) => setCard(selected)}
                    >
                        {paymentCards.map((p) => {
                            return <Picker.Item key={p.name} label={p.name} value={p.name} />
                        })}
                    </Picker>
                ) : null
                }
